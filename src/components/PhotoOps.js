function applyBlur(data, canvas, radius) {
  let res = data;
  blurARGB(res, canvas, radius);
  return res;
}

// Apply division normalization
function divideImages(original, blurred, width, height) {
  const result = new Uint8ClampedArray(original.length);
  for (let i = 0; i < original.length; i += 4) {
    result[i] = Math.min(255, (original[i] * 255) / (blurred[i] + 1));
    result[i + 1] = Math.min(
      255,
      (original[i + 1] * 255) / (blurred[i + 1] + 1)
    );
    result[i + 2] = Math.min(
      255,
      (original[i + 2] * 255) / (blurred[i + 2] + 1)
    );
    result[i + 3] = original[i + 3]; // Alpha channel remains unchanged
  }
  return result;
}

// Apply unsharp masking (simple version)
function unsharpMask(data, blurred, width, height, radius, amount) {
  const result = new Uint8ClampedArray(data.length);
  for (let i = 0; i < data.length; i += 4) {
    result[i] = Math.min(255, data[i] * (1 + amount) - blurred[i] * amount);
    result[i + 1] = Math.min(
      255,
      data[i + 1] * (1 + amount) - blurred[i + 1] * amount
    );
    result[i + 2] = Math.min(
      255,
      data[i + 2] * (1 + amount) - blurred[i + 2] * amount
    );
    result[i + 3] = data[i + 3]; // Alpha channel remains unchanged
  }
  return result;
}
...
const level = (data, blackLevel = 25, whiteLevel = 75) => {
  for (let i = 0; i < data.length; i += 4) {
    for (let j = 0; j < 3; j++) {
      data[i + j] =
...
const threshold = (pixels, level = 0.5) => {
